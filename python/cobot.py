...
CMD_PORT = 5000 # 명령 하는 부분
DATA_PORT = 5001 # 상태 데이터 받는 부분 
...
def DisConnectToCB():
    global cmd_connect, data_connect
    global CMDSock, DATASock

    print("[INFO] Disconnecting robot...")

    time.sleep(1)  # 스레드에서 recv() 중일 수 있으므로 약간 대기
    ...
    try:
        CMDSock.close()
        print("[INFO] CMDSock closed.")
    except:
        pass

    try:
        DATASock.shutdown(SHUT_RDWR)
    except:
        pass
    try:
        DATASock.close()
        print("[INFO] DATASock closed.")
    except:
        pass
...
def SendCOMMAND(str, cmd_type): # 다른 곳에서 cmd_type으로는 MOVE(0), NONMOVE(1)를 사용
    global cmd_send_flag
    ...
    if cmd_type == CMD_TYPE.MOVE:
        while True:
            ...
            if IsIdle() & (bReadCmd == False):
                CMDSock.send(str_space.encode('utf-8')) # 전송!!!
                moveCmdFlag = True
                cmd_send_flag = 1
                systemstat_global.robot_state = 3
                return True
        ...
        # manualScript에서는 ex_msg, CMD_TYPE.NONMOVE를 사용 -> else로 빠짐 
        CMDSock.send(str_space.encode('utf-8')) # 전송!!!
        cmd_send_flag = 1
        return True
